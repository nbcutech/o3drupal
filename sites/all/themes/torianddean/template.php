...
function torianddean_preprocess_page(&$variables) {
...
function torianddean_preprocess_showsite_cast_fields(&$variables) {
  static $accounts;  
  ...
  if (is_null($accounts[$uid])) {
    ...
    //$variables['cast_member_picture'] = $variables['directory'] ."/images/blogs/blog_author_thumb/{$accounts[$uid]->field_cast_name_value}.jpg";
...
//this is NOT an API call. It's a utility to get info to add to the $variables array
function torianddean_showsite_blog_episode_image(&$variables) {
  $uid = $variables['fields']['uid']->content;
  ...
  if (is_null($img_path)) {
	...
	//$img_path = '/'. $variables['directory'] ."/images/blogs/blog_author_thumb/{$variables['cast_member_name']}.jpg?{$uid}";
...
function torianddean_preprocess_views_view__showsite_blogs__page_1(&$variables) {
  ...
  $account = db_fetch_object($cast_query);
    ...
    //$variables['cast_member_picture'] = $variables['directory'] ."/images/blogs/blog_author_thumb/{$account->field_cast_name_value}.jpg";
    $variables['cast_member_picture'] = "sites/all/themes/oxygen/images/blog_photo/uid".$uid."-".strtolower($accounts[$uid]->field_cast_name_value).".jpg"; 
...
function torianddean_preprocess_views_view_fields__showsite_blogs__page(&$variables) {
  torianddean_preprocess_showsite_cast_fields($variables);
  torianddean_showsite_blog_episode_image($variables);
}
...
function torianddean_preprocess_views_view_fields__showsite_blogs__page_1(&$variables) {
  torianddean_preprocess_showsite_cast_fields($variables);
  torianddean_showsite_blog_episode_image($variables);
}
...
function torianddean_preprocess_views_view_fields__showsite_blogs__block_1(&$variables) {
  torianddean_preprocess_showsite_cast_fields($variables);
...
function torianddean_preprocess_views_view_fields__showsite_blogs__attachment_1(&$variables) {
  torianddean_preprocess_showsite_cast_fields($variables);
}

function torianddean_preprocess_node($variables) {
