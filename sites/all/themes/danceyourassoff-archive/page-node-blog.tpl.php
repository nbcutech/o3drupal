...
?> 
	...
	.image-comment{width:430px; padding-left:0px;overflow:hidden;}
...
<?php
	...
	$dyao_blog_list[$latestn->value] = array('nid'=>$latestn->nid,'uid'=>$node->uid,'teaser'=>$latestn->teaser,'class'=>$highlight);

	}
    }
    //blog listings per person dyao

    //start contestants
    print '<img style="padding-left: 18px;" src="/sites/all/themes/danceyourassoff/images/blogs/blog_contestantsHDR.gif">';
 
    // you can edit this array to remove or add new blogs to listing for contestants.
    $contestants = array('Alicia','Angela','Miles','Pinky','Ruben','Shayla','Tara','Trice','Mara','Warren','Brandon','Karla');
    sort($contestants);
    foreach ($contestants as $contestant)     {
    $name = strtolower($contestant);
    print '<div class="blog_author_name"><a href="/blog/' . $dyao_blog_list[$contestant]['uid'] . '" class="blog_author_link">' . $name. '</a><div class="blog_teaser">' . $dyao_blog_list[$contestant]['teaser'] . '</div><div class="blog_link"><a href="/node/' . $dyao_blog_list[$contestant]['nid'] . '" class="' . $dyao_blog_list[$contestant]['class'] . '">Read ' . $contestant . '\'s blog</a></div>
</div>';
    }

    print '<img style ="padding-left:18px;"src="/sites/all/themes/danceyourassoff/images/blogs/host.gif"/>';
    
    // you can edit this array to remove or add new blogs to listing for trainers.
    $hosts = array('Marissa');
    sort($hosts);
    foreach ($hosts as $host)     {   
    $name = strtolower($host);
    print '<div class="blog_author_name"><a href="/blog/' . $dyao_blog_list[$host]['uid'] . '" class="blog_author_link">'.$name.'</a><div class="blog_teaser">' . $dyao_blog_list[$host]['teaser'] . '</div><div class="blog_link"><a href="/node/' . $dyao_blog_list[$host]['nid'] . '" class="' . $dyao_blog_list[$host]['class'] . '">Read ' . $host . '\'s blog</a></div>
</div>';
    }
    
    
    print '<img style ="padding-left:18px;"src="/sites/all/themes/danceyourassoff/images/blogs/blog_crunch_trainerHDR.gif"/>';

    // you can edit this array to remove or add new blogs to listing for trainers.
    $trainers = array('Lee');
    sort($trainers);
    foreach ($trainers as $trainer)     {   
    $name = strtolower($trainer);
    print '<div class="blog_author_name"><a href="/blog/' . $dyao_blog_list[$trainer]['uid'] . '" class="blog_author_link">'.$name.'</a><div class="blog_teaser">' . $dyao_blog_list[$trainer]['teaser'] . '</div><div class="blog_link"><a href="/node/' . $dyao_blog_list[$trainer]['nid'] . '" class="' . $dyao_blog_list[$trainer]['class'] . '">Read ' . $trainer . '\'s blog</a></div>
</div>';
    }
    
   print '<img style ="padding-left:18px;"src="/sites/all/themes/danceyourassoff/images/blogs/blog_dyao_challengeHDR.gif"/>';

    // you can edit this array to remove or add new blogs to listing for judge.
    $judges = array('Mary');
    sort($judges);
    foreach ($judges as $judge)     {   
    $name = strtolower($judge);
    print '<div class="blog_author_name"><a href="/blog/' . $dyao_blog_list[$judge]['uid'] . '" class="blog_author_link">'.$name.'</a><div class="blog_teaser">' . $dyao_blog_list[$judge]['teaser'] . '</div><div class="blog_link"><a href="/node/' . $dyao_blog_list[$judge]['nid'] . '" class="' . $dyao_blog_list[$judge]['class'] . '">Read ' . $judge . '\'s blog</a></div>
</div>';
    }
