...
function thegleeproject_preprocess_page(&$variables) {
...
function thegleeproject_preprocess_showsite_cast_fields(&$variables) {
  static $accounts;  
...
//this is NOT an API call. It's a utility to get info to add to the $variables array
function thegleeproject_showsite_blog_episode_image(&$variables) {
  $uid = $variables['fields']['uid']->content;
...
function thegleeproject_preprocess_views_view__showsite_blogs__page_1(&$variables) {
...
function thegleeproject_preprocess_views_view_fields__showsite_blogs__page(&$variables) {
  thegleeproject_preprocess_showsite_cast_fields($variables);
  thegleeproject_showsite_blog_episode_image($variables);
}
...
function thegleeproject_preprocess_views_view_fields__showsite_blogs__page_1(&$variables) {
  thegleeproject_preprocess_showsite_cast_fields($variables);
  thegleeproject_showsite_blog_episode_image($variables);
}
...
function thegleeproject_preprocess_views_view_fields__showsite_blogs__block_1(&$variables) {
  thegleeproject_preprocess_showsite_cast_fields($variables);
...
function thegleeproject_preprocess_views_view_fields__showsite_blogs__attachment_1(&$variables) {
  thegleeproject_preprocess_showsite_cast_fields($variables);
}

function thegleeproject_preprocess_node($variables) {
  ...
  $account = db_fetch_object($cast_query);
   ...
   $node = $variables['node'];
  if($node->nid == 98709) {
        drupal_add_css(drupal_get_path('theme','thegleeproject')."/thegleeproject_contest.css", 'theme');
        drupal_add_js(drupal_get_path('theme','thegleeproject')."/jquery.cookie.js", 'module');
        //drupal_add_js(drupal_get_path('theme','thegleeproject')."/thegleeproject_contest.js", 'theme');
