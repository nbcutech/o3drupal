...
function storibookweddings_preprocess_page(&$variables) {
...
function storibookweddings_preprocess_showsite_cast_fields(&$variables) {
  static $accounts;  
...
//this is NOT an API call. It's a utility to get info to add to the $variables array
function storibookweddings_showsite_blog_episode_image(&$variables) {
  $uid = $variables['fields']['uid']->content;
...
function storibookweddings_preprocess_views_view__showsite_blogs__page_1(&$variables) {
...
function storibookweddings_preprocess_views_view_fields__showsite_blogs__page(&$variables) {
  storibookweddings_preprocess_showsite_cast_fields($variables);
  storibookweddings_showsite_blog_episode_image($variables);
}
...
function storibookweddings_preprocess_views_view_fields__showsite_blogs__page_1(&$variables) {
  storibookweddings_preprocess_showsite_cast_fields($variables);
  storibookweddings_showsite_blog_episode_image($variables);
}
...
function storibookweddings_preprocess_views_view_fields__showsite_blogs__block_1(&$variables) {
  storibookweddings_preprocess_showsite_cast_fields($variables);
...
function storibookweddings_preprocess_views_view_fields__showsite_blogs__attachment_1(&$variables) {
  storibookweddings_preprocess_showsite_cast_fields($variables);
}

function storibookweddings_preprocess_node($variables) {
  ...
  $account = db_fetch_object($cast_query);
    ...
    $variables['cast_member_short_bio'] = $account->field_short_bio_value;
		...
		if($node->nid==103735) {
	  	...
	  	drupal_add_css(drupal_get_path('theme','storibookweddings')."/storibook_contest.css", 'theme');
