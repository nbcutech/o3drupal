...
function oxy_channel_finder_action(){  //console.log('fire');
	...
	if(oxy_channel_finder_validate()){
		$jQuery15.ajax({ 
			type: "GET",
			url: '/sites/all/themes/theworldaccordingtoparis/channel-finder.php?zip=' + $jQuery15('#zipcode').val(),
			dataType: "xml",
			success: function(xml) { //console.log(xml);
				$jQuery15(xml).find('provider').each(function(){
					var provider = oxy_channel_finder_trim($jQuery15(this).find('name').text());
					var status = oxy_channel_finder_trim($jQuery15(this).find('status').text());
					var channel = oxy_channel_finder_trim($jQuery15(this).find('channel').text());
					var hdchannel = oxy_channel_finder_trim($jQuery15(this).find('hdchannel').text()); //console.log(provider + '-' + channel);
					...
					response = response + '<div><div class="provider">' + provider + '</div><div class="channel">' + ch + hd + '</div><div class="clear">&nbsp;</div></div>';
				});
