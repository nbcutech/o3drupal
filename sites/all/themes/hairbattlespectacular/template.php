...
function hairbattlespectacular_preprocess_page(&$variables) {
...
function hairbattlespectacular_preprocess_showsite_cast_fields(&$variables) {
  static $accounts;  
...
//this is NOT an API call. It's a utility to get info to add to the $variables array
function hairbattlespectacular_showsite_blog_episode_image(&$variables) {
  $uid = $variables['fields']['uid']->content;
...
function hairbattlespectacular_preprocess_views_view__showsite_blogs__page_1(&$variables) {
...
function hairbattlespectacular_preprocess_views_view_fields__showsite_blogs__page(&$variables) {
  hairbattlespectacular_preprocess_showsite_cast_fields($variables);
  hairbattlespectacular_showsite_blog_episode_image($variables);
}
...
function hairbattlespectacular_preprocess_views_view_fields__showsite_blogs__page_1(&$variables) {
  hairbattlespectacular_preprocess_showsite_cast_fields($variables);
  hairbattlespectacular_showsite_blog_episode_image($variables);
}
...
function hairbattlespectacular_preprocess_views_view_fields__showsite_blogs__block_1(&$variables) {
  hairbattlespectacular_preprocess_showsite_cast_fields($variables);
...
function hairbattlespectacular_preprocess_views_view_fields__showsite_blogs__attachment_1(&$variables) {
  hairbattlespectacular_preprocess_showsite_cast_fields($variables);
}

function hairbattlespectacular_preprocess_node($variables) {
  ...
  $account = db_fetch_object($cast_query);
    ...
    $node = $variables['node'];
    if($node->nid == 102991) {
        drupal_add_css(drupal_get_path('theme','hairbattlespectacular')."/hairbattle_contest.css", 'theme');
